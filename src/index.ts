...
const sleepFaker = (ms: number = 2000) => new Promise((resolve) => setTimeout(resolve, ms));
...
if (options.init) {
  ...
  const framework = await select({
    ...
    choices: [
      ...
      },
      {
        ...
        value: "vite",
        description:
          "vite is a build tool that provides a fast and lean development experience",
      },
      new Separator(),
      {
        name: "nuxtjs",
  ...
  if (framework === "nextjs") await bootStrap(name);
